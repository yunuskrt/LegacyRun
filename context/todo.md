...
- [ ] 🟡 **Phase 5 — Draft Page Design (Mock Data)**: **UI only** — build and style the `/play/draft` page — team+season card, roster list, formation slots, reroll counter — driven entirely by the `src/data/` mock dataset. Static markup and styling, no interactivity or state. — **in progress**: part 1 of 2 done (route, Dark Trophy Room theme, top bar, responsive two-column shell with placeholder panels); part 2 covers the court graphic, player cards, roster reveal, reroll pool, motion, and enabled/disabled states.
