...
- [ ] 🟡 **Phase 7 — Basketball-Reference Scraper** (In Progress): one rerunnable ingestion script covering both rosters/team-seasons and player-season stats — they come off the same source pages, so they are not split. Per-season CSV exports are uploaded by hand into the root `data/raw` folder (raw, untouched, one or more files per season); the script parses them and converts them to normalized JSON, which is the artifact later phases consume. Nothing is written to Neon here.
