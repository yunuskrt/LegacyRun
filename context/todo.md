...
- [ ] 🟡 **Phase 10 — Team Rating Aggregation & Ingestion Runner**: Neon Setup, .ts files formed for db tables, load validated, rated data into Neon. — **in progress**: part 1 (team rating engine + all seven `src/data/db/` table files, playoff participation included, via `scripts/build-db-data.mts`) is built; part 2 (Neon setup and the ingestion runner) is not started. Full spec: `context/features/phase-10-team-rating-data-ingestion-01.md`.
