...
Closed the three share-card candidates in `context/docs/security-scan-findings.md` before Phase 23, under the standing constraint that **no authentication is being added**: the crests are read off disk instead of fetched, the card render is throttled per IP and capped at two concurrent rasterisations, the four Neon-backed routes are throttled, the share payload is length-bounded before decoding, and the PNG carries `nosniff`.
**Finding #1 was deleted rather than mitigated.** `loadShareLogos(slugs, origin, fetchLogo)` became `loadShareLogos(slugs, readLogo)`: with no outbound fetch there is no `Host`-header SSRF left to reason about, and five network round trips leave every render, which is also the cheapest available win against #2. `TEAM_SLUG_PATTERN` moved into `team-logo.ts` beside the path convention it has to stay safe for, so `share-card.ts`'s Zod schema and the reader's own guard are one definition — the reader re-asserts it rather than trusting its only current caller, because it is what turns a slug into a path on disk.
**`public/` is not traced into a serverless bundle**, so `readFile` works in dev and finds nothing in production. `outputFileTracingIncludes: { "/api/share/card": ["./public/logos/**"] }` pulls the 1.0MB of crests in — verified against `node_modules/next/dist/docs/.../output.md`, not from memory. The failure mode is deliberately safe: `loadShareLogos` answers `null` for anything unreadable and the card draws initials, so a bad trace degrades crests rather than 500-ing. **Still unverified on a real deploy** — dev does not exercise tracing. Render one card on the first preview and look for crests.
**Two layers, because only one of them is honest.** `X-Forwarded-For` is caller-supplied, so the per-IP budgets (60/min data, 20/min card) bound accidents and casual abuse and nothing more — rotate the header and they are gone. The concurrency gate is what holds, because it counts work in flight instead of trusting an identifier. Verified live: six simultaneous renders returned `429 429 429 429 200 200`.
**A fixed window is not a ceiling.** The window opens on a caller's first request, not a clock boundary, so draining it at the last instant and again as it rolls over passes **39 requests inside 1ms** against a limit of 20 — measured, after two earlier measurements wrongly reported "no burst" because the window had been opened at the wrong time. Inherent to fixed windows and left alone, since the gate bounds the expensive path regardless.
**The store is swept because its key comes from a header.** A caller inventing keys would otherwise grow the map without bound; past `MAX_TRACKED_KEYS` it drops finished windows and, failing that, empties — the limiter fails open rather than exhausting memory.
**Signing the payload (#3) is a deliberate no.** The card asserts a single-player result with no accounts and no leaderboard, so forging one crosses no trust boundary; worse, the payload is built on the client, so an HMAC needs either a secret in the bundle or a second unauthenticated endpoint minting signed URLs. The narrow real part — caller-chosen text rendered into a PNG on our origin — is bounded by the throttles and `nosniff` instead. What did change is a 2048-character ceiling on `d` before `atob` and `JSON.parse` see it; a test proves a schema-maximal card encodes well under it, so the ceiling can never reject a real one.
**`isApiError` was a hand-written predicate and silently dropped the new code.** A throttled player would have read "Couldn't reach the draft pool" — `ApiError` now derives from an `API_ERRORS` list, so the runtime narrowing and the type cannot drift, and `npm run build` (not vitest, which does not typecheck) is what surfaced the three client message maps that needed `RATE_LIMITED` copy.
**A file-parsing guard can satisfy itself — again.** The first route-coverage test asserted `source.includes("DATA_ROUTE_BUDGET")` and passed after the rate-limit call was deleted, because the import stayed behind. It now matches the call site. The same trap is already recorded in the Momentum Polarity entry; it caught a second author anyway.
Mutation-checked against a passing control: dropping either slug guard fails 1, dropping the payload ceiling fails 1, dropping the tracing include fails 1, dropping the sweep fails 2, an ungated gate fails 3, `leaveGate` below empty fails 1, budgets sharing one key fails 1, the retry-after floor fails 1, an unthrottled data route fails 2, the card route on the data budget fails 1, `leaveGate` outside `finally` fails 1, a slug rule not matching real data fails 3, and a crest path pointing nowhere fails 2. The payload-ceiling and budget-namespacing tests were both **fake on the first cut** — junk of that length fails `atob` anyway, and spending the small budget first stays under the large one — and were rewritten until they discriminated.
Verified against a running server, not only asserted: a full run's traffic from one IP (16 data calls plus three renders) came back all 200, which is the check the "users should not be overwhelmed" constraint actually turns on. The rendered card also reached the **CHAMPION** path the Shareable Run Card entry left unverified, with all five crests drawn from disk and `LAL` deduped to a single read.
**Known gap:** a throttled card shows "That card could not be rendered. Try another shape." The dialog previews through `<img onError>`, which cannot read a status code; distinguishing 429 means rewriting the preview as a `fetch`. At 20/min against a dialog that spends 3, no real user reaches it.

### Fix — Pre-Deploy UI Pass

Six defects from a live browser review of the **production build** at 375 / 768 / 1280, closed before Phase 23: the draft route's sideways scroll, an unreachable 36px `Get Random Team`, a clipped line-score total, focus dropped to `<body>` by both dialogs, icon-only reroll buttons, and a truncated season in the confirm dialog. Five were mobile-only; the focus defect applied at every width.
**Every one was invisible from the desktop viewport the app is usually developed in**, and two were mis-attributed before being measured. The court was blamed for the overflow and is innocent — its slots all sit inside the box (`0.2%` → `99.8%`). The real cause is `DraftExperience`'s grid declaring columns only at `lg`: below it the single implicit column is `auto`, whose floor is **min-content**, so the widest player-name card sets the width and the `w-full` court stretches past the viewport. **The overflow is therefore roster-dependent** — short names measured 6px, `M. Ray Richardson` measured 34px — so one clean load proves nothing. `grid-cols-[minmax(0,1fr)]` at the base breakpoint pins it; a synthetic 38-character name cannot move the column off `342.667px`.
**Goal 2 was arithmetically impossible as written and had to change shape.** Fixed chrome above the board card is 585px at 375×667, plus 20px card padding, against a 44px button that must start by 623 — 18px of room for an empty state that occupies 156. Deleting the entire empty state still lands six pixels short. Raising the height moved the button not at all (`top: 805`, both before and after), and the court cannot be capped by height without overriding its aspect ratio. The CTA is pinned below `lg` instead, reusing `ReplayControlBar`'s pattern: `top: 611`, fully visible, with the in-card button `hidden lg:inline-flex` so exactly one CTA renders at any width.
**The pinned bar is keyed on `canOfferTeam`, not `canGetTeam`** — the latter differs by `!isFetchingTeam` and blinks the bar out and back mid-fetch. That rule was first re-derived inline as `!isComplete && !team` and inlined a third time for the padding reservation, before turning out to be `canOfferTeam` verbatim; all three now resolve to one call, and mutation-checking proved the existing tests really do pin it (dropping either term fails 1 and 2 tests against a passing 36-test control). **`fixed` takes no flow space**, so `main` carries `pb-20 lg:pb-0` while the bar is up and releases it at 5/5.
**A first cut of that bar swapped its label to `Drawing a team` during the fetch — and the card directly above already says exactly that, with a spinner.** Measured: the same phrase twice, ~100px apart, for ~400ms of every ~1s fetch. Caught in review, not by a test. One status, one action: the label now stays put and only the disabled state changes.
`min-w-md` on the line score is `28rem` = 448px inside a 343px box, which is why `Q4` and the total sat off-screen behind an `overflow-x-auto` that was doing its job. Dropping the floor and tightening cells to `px-2 sm:px-4` fits all six columns in 343px; swept across 1–6 OT on a clone, it fits through 3 OT and scrolls beyond, and **no header wraps and no cell clips at any OT count**. Desktop is untouched — that container is 560px, wider than the floor ever was.
**The two dialogs needed different fixes for the same defect.** `ShareRunDialog` took `DialogTrigger asChild` around the existing `motion.button`, which let its `open` state be deleted outright and hands focus restoration to Radix. `SquadConfirmDialog` could not: it is gated on `!isHandingOff`, so moving the trigger inside would unmount the button during handoff — it restores focus by hand through a `triggerRef` in `onCloseAutoFocus`. Both verified keyboard-only, `Enter` → `Escape`, not by click.
Item 6 was a convention drift, not a layout bug: the dialog was the only place in the codebase rendering `{teamName} · {season}`, so `truncate` ate the era. `RunSquadGrid` and `SquadRail` already put the season first, and matching them keeps the half the crest cannot convey. **Extracting that shared string into a helper was considered and rejected** — the "logic" is a space, the five call sites take four differently-shaped inputs, and the riskiest consumer is Satori's `ShareCardImage`.
No new tests: `src/lib/` is unchanged and this repo scopes Vitest to utilities, while every layout claim here needs real layout that neither the node environment nor jsdom provides. The verification is browser measurement, re-run end to end after each change — all three draft paths, a failed fetch that toasts without consuming a reroll, a full run to the result screen, the share card at 1080×1920, and zero console messages throughout. **A `browser_drag` that failed once and passed on retry was tool flakiness, not a regression** — worth knowing before the next author reports it as one.
**Left open deliberately**, for `todo.md` rather than this pass: the bracket archive reading `AWAITING WINNER` under "The run is complete", no link back to `/` from `/play/draft` (zero `<a>` elements on the page), the 28×28 dialog close `X`, the 40px `Play first round`, the weak focus ring on player cards, and losing a run on reload.

### Chore — Untrack the `src/data` Pipeline Files

`src/data/db/`, `src/data/raw/` and `src/data/rating/season_players.ts` are gitignored and untracked — 101 files, ~18MB — with `tsconfig.build.json` keeping `next build` green on a checkout that has none of them. No app code changed.

**`.gitignore` on a tracked file is a no-op.** All 101 were already committed, so the ignore rules do nothing without `git rm -r --cached`, which drops the index entry and leaves the working tree alone. **Never `rm` these.**
**The failure mode is the TypeScript step, not the runtime** — on Vercel a failed deployment, with the previous one still serving, never a crash. `tsconfig.json` sets `include` to `**/*.ts` / `**/*.mts` excluding only `node_modules`, so `next build` typechecks tests and scripts; Next's docs say so outright — *"The CLI checks the complete project selected by your `tsconfig` file. This includes test files."* Measured on a copy of the tree with the three trees gone: **87 errors across 7 files**, `db-data.test.ts` 34, `landing.test.ts` 18, `ingest-db-data.mts` 14, `match-data.test.ts` 10, `team-logo.test.ts` 5, `bracket-data.test.ts` 4, `build-db-data.mts` 2.
**Nothing here was ever in the deployed output**, so this shrinks no function and no full clone — the blobs stay in history. Only `formations.ts` is runtime-reachable (`play/draft/page.tsx` via the `@/data` barrel, `share-card.ts` directly), and it is in none of the three trees. Confirmed against real build output: `Alaa Abdelnaby`, the first row of `db/player.ts`, appears in zero production chunks, and the two chunks matching `player_season_team` are Prisma's embedded schema naming the *table*, not the file. The win is repo hygiene and build-clone time.
**Deletion was rejected in favour of one config file.** `tsconfig.build.json` extends `tsconfig.json` and excludes the eight data-dependent files; `typescript.tsconfigPath` in `next.config.ts` points the build at it. Exclusion works only because **nothing imports those eight** — an excluded file that something imported would still be pulled in. Deleting them instead would have cost ~75 tests and the whole ingestion pipeline permanently, to fix a problem that exists only where the data is absent.
**Ignore `rating/season_players.ts`, never `rating/`** — the directory also holds `season-players.test.ts`, which the broader rule would silently untrack.
**Untracking is recoverable and an earlier draft of the spec was wrong to call archiving blocking.** `git show 97ce336:src/data/db/player.ts` still prints the file; only a history rewrite makes it permanent. It matters because `db/` is the input to the 🔒 `db:ingest`, the one path back if Neon is lost, and `build:db-data` / `rate:players` are themselves locked and need `raw/`.
**`git rm --cached` spares the working tree only on the branch that runs it — merging the result into a branch that still tracks the files deletes them off disk.** `main` had all 101 tracked, so the merge applied the deletion and took them; the full-tree typecheck and six suites failed on `main` seconds later, on a tree that had been green on the branch. Recovered with `git archive 97ce336 <paths> | tar -x`, which writes the worktree without touching the index — `git checkout <rev> -- <paths>` would have staged all 101 straight back in. **Any other checkout that predates this merge loses the same files the same way**, so re-extract there rather than assuming a pull is harmless.
**Review found the exclude list had no guard, and it rots silently in both directions** — a new data import fails only on Vercel, while a stale entry leaves that file never type-checked again with no symptom at all. `src/lib/tsconfig-build.test.ts` asserts set equality both ways plus a non-empty floor, since two empty sets satisfy both comparisons. It scans for the imports it describes, so it **skips itself by path** — the "a file-parsing guard can satisfy itself" trap now recorded for the third time in this file.
`typescript.tsconfigPath` governs `next dev` as well as `next build`, so those eight files are unchecked in development too; `npm run typecheck` was added to cover the whole tree and needs the data present.
Verified by moving the data off disk rather than by argument: **build exit 0** with all three absent, and **exit 1** with `tsconfigPath` removed under the same conditions, failing with exactly the `TS2307` cascade. 742 tests / 33 files pass with the data back (739 before, +3 from the guard, none lost), full-tree typecheck and lint clean, and a production smoke returned 200 on `/`, `/play/draft`, `/play/tournament` and `/api/draft/team` with real Neon rows. The guard was mutation-checked against a passing 3-test control: dropping an entry fails 1, a stale entry fails 1, breaking the detector fails 2.
**Accepted, not defects:** a fresh clone without the data fails six suites (`db-data`, `landing`, `match-data`, `team-logo`, `bracket-data`, `season-players`), and `landing.test.ts` — the only thing binding the home page's counters to the real tables — along with `db-data.test.ts`'s `project-overview.md` hard constraints can no longer be checked anywhere but a machine holding the data. `CLAUDE.md` records where it lives. Five pre-existing `format:check` warnings were left alone, each confirmed already failing on `main`.
