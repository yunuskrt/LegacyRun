...
- Open decisions to make explicitly during implementation, and record in History:
  ...
  - **ID format.** Real rows use cuids. Fixtures should use readable, stable, deterministic ids (`"lebron-james"`, `"cle-2010"`, `"lebron-james-2010"`) rather than fake cuids — the shapes are `string` either way, and readable ids make the Phase 5 UI debuggable. Note the divergence from production ids so nothing later assumes cuid formatting. **Superseded in part by Phase 9:** `players` and `teams` are keyed by `slug`, not a cuid — only the season-scoped tables keep generated ids.
